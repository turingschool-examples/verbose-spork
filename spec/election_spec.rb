...
RSpec.describe Election do
  ...
  it 'lists winners of each race, assuming the respective race(s) are closed' do
    @election.add_race(@race_texas)
    @election.add_race(@race_california)
    ...
    expect(@election.winners()).to eq([@candidate_1, @candidate_4])

    #Verify it updates correctly (e.g. vote recount)
    @candidate_2.vote_for
    @candidate_2.vote_for

    expect(@election.winners()).to eq([@candidate_2, @candidate_4])
